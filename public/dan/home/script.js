		...
		await fetch('/ward')
			...
			.then((wards) => {
				wards.forEach(async (item) => {
					if (removeVietnameseTones(item.name) == removeVietnameseTones(ward.replace('Phường', '').trim()) && removeVietnameseTones(item.district.name) == removeVietnameseTones(district.replace('Quận', '').trim())) {
						clickMarkerInfo += `
